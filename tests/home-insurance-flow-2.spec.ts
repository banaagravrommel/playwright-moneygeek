...
class HomeInsuranceQuotePage {
  ...
  async selectNoCurrentInsurance() {
    await this.page.locator('label').filter({ hasText: 'NoI either don\'t have or' }).click();
  }
...
test.describe('Home Insurance Quote Flow with Validations', () => {
  test('should complete home insurance quote form with element validations at each step', async ({ page }) => {
    ...
    // Step 5: Validate and select no current insurance
    await quotePage.validateCurrentInsuranceSection();
    await quotePage.selectNoCurrentInsurance();
    
    // Step 6: Validate spouse section and confirm continuous insurance
    await quotePage.validateSpouseSection();
    await quotePage.confirmContinuousInsurance();
    
    // Step 7: Validate personal info section and fill details
    ...
    // Step 8: Validate email section and fill email
    ...
    // Step 9: Validate results page elements
